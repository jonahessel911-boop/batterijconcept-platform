...
export async function GET(
  ...
  try {
    ...
    const { data: orders, error } = await sb
      .from("projecten")
      .select(
        "id, project_nummer, titel, status, schouw_at, schouw_jaar, schouw_week, schouw_notities, installatie_at, installatie_notities, notities, monteur, startdatum, opleverdatum, created_at, leads(naam, telefoon, email, postcode, huisnummer, toevoeging, straat, plaats, lead_number)"
      )
